...
def download_selenium(driver, interested_price):
    ...
    underlying_index_text = driver.find_element_by_css_selector('#optionchain_equity_sp > div > div > div:nth-child(1) > div.col-md-6.text-right > span:nth-child(1)').text
    ...
    for index, row in interested_rows.iterrows():
        new_rows.append(
            [item for item in row.values])  # iterrows returns arrays, which need to be iterated through as well
    ...
    # Open the output sheet and write data row by row
    wb = xlsxwriter.Workbook('scrape_output.xlsx')
    sheet = wb.add_worksheet()
    number_format = wb.add_format({'num_format': '#,##0.00'})
    number_format = wb.add_format({'num_format': '#,0'})

    for row_num, row_data in enumerate(rows_to_write):
        for col_num, cell_data in enumerate(row_data):
            try:
                sheet.write(row_num, col_num, int(cell_data))
            except:
                try:
                    sheet.write(row_num, col_num, float(cell_data))
                except:
                    sheet.write(row_num, col_num, cell_data, number_format)
